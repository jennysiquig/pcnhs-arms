    ...
    if($resultCheckPw->num_rows>0) {
        while ($row=$resultCheckPw->fetch_assoc()) {
         ...
         if($verifypw = true) {
            ...
            SET `uname`='$uname', `password`='$new_password',`hashed_pw` = '$hashed', `last_name`='$last_name', `first_name`='$first_name', `mname`='$mname', `position`='$position', `access_type` = '$access_type',`accnt_status`='$accnt_status' 
